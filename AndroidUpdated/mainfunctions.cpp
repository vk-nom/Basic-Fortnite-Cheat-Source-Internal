...
namespace AndroidUpdated {
	...
	ImGuiWindow& BeginScene() {
		...
		ImGui::Begin(xorstr("##BeginScene"), nullptr, ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoTitleBar);
	...
	VOID EndScene(ImGuiWindow& window)
	{
		...
		if (showMenu)
		{
			...
			{
				ImGui::Text(xorstr("Hey I'm there https://github.com/Android1337/Basic-Fortnite-Cheat-Source-Internal"));
	...
	HRESULT __stdcall hk_present(IDXGISwapChain* pSwapChain, UINT sync_interval, UINT flags)
	{
		...
		do {
			...
			//Get the GameInstance address
			DWORD_PTR Gameinstance = ReadDWORD_PTR(AndroidNiceThings::Uworld, GameinstanceOffset);
			//if it can't get the GameInstance address then break the loop
			if (!Gameinstance) break;

			//Get LocalPlayers address
			DWORD_PTR LocalPlayers = ReadDWORD_PTR(Gameinstance, LocalPlayersOffset);
			...
			//Get the PlayerController address
			AndroidNiceThings::PlayerController = ReadDWORD_PTR(AndroidNiceThings::Localplayer, PlayerControllerOffset);
			//if it can't get the PlayerController address then break the loop
			if (!AndroidNiceThings::PlayerController) break;

			//Get the LocalPawn pointer
			auto LocalPawnInAutoVariable = ReadPointer(AndroidNiceThings::PlayerController, LocalPawnOffset);
			//if it can't get the LocalPawn pointer then break the loop
			if (!LocalPawnInAutoVariable) break;

			//Get the LocalPawn address
			AndroidNiceThings::LocalPawn = ReadDWORD_PTR(AndroidNiceThings::PlayerController, LocalPawnOffset);
			//if it can't get the LocalPawn address then break the loop
			if (!AndroidNiceThings::LocalPawn) break;
			
			
			//Get the PlayerState address
			AndroidNiceThings::PlayerState = ReadDWORD_PTR(AndroidNiceThings::LocalPawn, PlayerStateOffset);  //std::cout << "PlayerState: " << AndroidNiceThings::PlayerState << "  |  ";
			if (!AndroidNiceThings::PlayerState) break;

			//Get the RootComponent address with LocalPawn + 0x130
			AndroidNiceThings::Rootcomp = ReadDWORD_PTR(AndroidNiceThings::LocalPawn, RootComponentOffset); //std::cout << "Rootcomp: " << AndroidNiceThings::Rootcomp << "  |  ";
			if (!AndroidNiceThings::Rootcomp) break;


			//Define a simple vector to do later a cache system
			std::vector<PVOID> PawnList;


			//Here we have our first for, that is used to loop the levels
			for (auto li = 0UL; li < ReadDWORD(AndroidNiceThings::Uworld, LevelsOffset + sizeof(PVOID)); ++li) {
				//Here we define the levels
				auto levels = ReadPointer(AndroidNiceThings::Uworld, LevelsOffset);
				if (!levels) break;
				...
				for (auto ai = 0UL; ai < ReadDWORD(level, 0x98 + sizeof(PVOID)); ++ai) {
					...
					if (!pawn || pawn == LocalPawnInAutoVariable) continue;

					//Here we define the player object name
					auto PawnObjectName = worker::GetObjectName((uintptr_t)pawn);

					//Here we check if the player object name is equal to a "real player"
					if ((PawnObjectName.find("PlayerPawn_Athena_C") != std::string::npos) || (PawnObjectName.find("PlayerPawn_Athena_Phoebe_C") != std::string::npos) || (PawnObjectName.find("BP_MangPlayerPawn") != std::string::npos)) {
						//Then we push back the pawn
			...
			for (auto EntityPawn : PawnList) {

				//Getting EntityMesh address with pawn + 0x280
				DWORD_PTR player_mesh = ReadDWORD_PTR(EntityPawn, MeshOffset); //mesh
	...
	BOOLEAN Start()
	{
		...
		if (FAILED(D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, 0, 0, &featureLevel, 1, D3D11_SDK_VERSION, &sd, &swapChain, &device, nullptr, &context))) {
			...
			MessageBox(0, xorstr(L"The process hasn't DirectX 11"), xorstr(L"Error"), MB_OK);
			return FALSE;
		}
